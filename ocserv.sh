...
Download_ocserv(){
	mkdir "ocserv" && cd "ocserv"
	    wget "https://raw.githubusercontent.com/fctx/one-oc/main/ocserv-1.5.0.tar.xz"
	[[ ! -s "ocserv-${ocserv_ver}.tar.xz" ]] && echo -e "${Error} ocserv 源码文件下载失败 !" && rm -rf "ocserv/" && rm -rf "ocserv-${ocserv_ver}.tar.xz" && exit 1
	tar -xJf ocserv-1.5.0.tar.xz && cd ocserv-1.5.0
	./configure
	make
	make install
	cd .. && cd ..
	...
	if [[ -e ${file} ]]; then
		mkdir "${conf_file}"
		wget --no-check-certificate -N -P "${conf_file}" "https://raw.githubusercontent.com/fctx/two-oc/master/ocserv.conf"
